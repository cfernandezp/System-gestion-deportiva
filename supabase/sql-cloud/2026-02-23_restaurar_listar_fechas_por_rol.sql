-- ============================================
-- RESTAURAR: listar_fechas_por_rol
-- Fecha: 2026-02-23
-- Motivo: Fix hora incorrecta. Los timestamps se almacenan
--         como hora Peru directamente en el campo timestamptz.
--         NO se debe aplicar AT TIME ZONE porque resta 5 horas
--         (mostraba 16:00 en vez de 21:00).
-- Solucion: Usar TO_CHAR sin conversion de timezone para
--           fecha_hora_inicio, fecha_formato y hora_formato.
-- ============================================
    ...
    IF p_seccion = 'proximas' THEN
        ...
        FROM (
            SELECT
                json_build_object(
                    'id', f.id,
                    'fecha_hora_inicio', TO_CHAR(f.fecha_hora_inicio, 'YYYY-MM-DD"T"HH24:MI:SS'),
                    'fecha_formato', TO_CHAR(f.fecha_hora_inicio, 'DD/MM/YYYY'),
                    'hora_formato', TO_CHAR(f.fecha_hora_inicio, 'HH24:MI'),
                    'lugar', f.lugar,
...
-- Comentario
COMMENT ON FUNCTION listar_fechas_por_rol IS 'E003-HU-009: Lista fechas con visibilidad por rol. FIX 2026-02-23: timestamps se almacenan como hora Peru directamente, sin conversion AT TIME ZONE.';
